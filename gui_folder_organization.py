...
def create_gui():
    ...
    choose_organize_by_type = ttk.Radiobutton(sorting_type_frame, text = "Organize files by type", variable = chosen_type, value = 1)
    choose_organize_by_type.pack(pady = 3)

    choose_organize_by_date = ttk.Radiobutton(sorting_type_frame, text = "Organize files by date", variable = chosen_type, value = 2)
    ...
    include_subdirs_check = ttk.Checkbutton(settings_frame, text = "Include Subdirectories", variable = include_subdirs_var, bootstyle = "round-toggle")
    include_subdirs_check.pack(pady = 3)

    #Checkbox for dry run
    dry_run_var = ttk.BooleanVar()
    dry_run_check = ttk.Checkbutton(settings_frame, text = "Dry Run", variable = dry_run_var, bootstyle = "round-toggle")
    ...
    directory_entry = ttk.Entry(centered_frame, width = 50, textvariable = directory_var)
    directory_entry.pack(side = LEFT, pady = 3, padx = (5, 0))

    browse_button = ttk.Button(centered_frame, text = "Browse", command = lambda: select_directory(directory_entry))
    ...
    log_text = ttk.Text(log_frame, height = 5, state = 'disabled')
    log_text.pack(fill = "both", expand = True, padx = 5, pady = 5)

    #Progress bar var
    progress_bar = ttk.Floodgauge(root, orient = HORIZONTAL, length = 300, mode = 'determinate', bootstyle = SUCCESS)
    ...
    organize_button = ttk.Button(buttons_frame, text = "Organize Files", command = lambda: run_gui_mode(chosen_type, progress_bar, undo_button, root, directory_entry, log_text, include_subdirs_var, dry_run_var))
